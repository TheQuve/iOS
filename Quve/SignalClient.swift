...
class SignalClient {
    ...
    init() {
        socket = WebSocket(url: URL(string: serverURL)!, protocols: [])
    }
    
    func send(sdp: RTCSessionDescription){
        let message = Message(type: .sdp, payload: sdp.jsonString() ?? "")
        if let dataMessage = try? JSONEncoder().encode(message), let stringMessage = String(data: dataMessage, encoding: .utf8){
            socket.write(string: stringMessage)
        }
    }
    
    func send(candidate: RTCIceCandidate){
        let message = Message(type: .candidate, payload: candidate.jsonString() ?? "")
        if let dataMessage = try? JSONEncoder().encode(message), let stringMessage = String(data: dataMessage, encoding: .utf8){
            socket.write(string: stringMessage)
        }
    }
